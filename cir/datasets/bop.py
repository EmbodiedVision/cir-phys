...
# https://github.com/princeton-vl/Coupled-Iterative-Refinement/blob/c50df7816714007c7f2f5188995807b3b396ad3d/datasets/bop.py,
...
from cir.utils import MEMORY
from cir.utils.augmentation import *
...
# --------------
# Code block modified compared to the original CIR version: accept split as arg and use it if the dataset is synpick
def keep_bop19(frame_index, ds_dir, split):
    targets = pd.read_json(ds_dir / f'{split if ds_dir.name == "synpick" else "test"}_targets_bop19.json')
...
# --------------
# Code block added compared to the original CIR version
def keep_no_gripper(frame_index, ds_dir, split):
    targets = pd.read_json(ds_dir / f'{split if ds_dir.name == "synpick" else "test"}_targets_no_gripper.json')
    targets = remap_bop_targets(targets)
    targets = targets.loc[:, ['scene_id', 'view_id']].drop_duplicates()
    index = frame_index.merge(targets, on=['scene_id', 'view_id']).reset_index(drop=True)
    assert len(index) == len(targets)
...
class BOPDataset:
    ...
    # --------------
    # Code block modified compared to the original CIR version: added arg only_no_gripper
    def __init__(self, ds_dir, split, use_augmentation=False, only_bop19_test=False, only_no_gripper=False,
                 use_pre_interpolated_depth=False, load_depth=True, check_image_exists=False, visib_fract_minimum=-1.0):
        # -------------
        ...
        if (ds_dir, split) in BOPDataset.frame_index:
            ...
            # --------------
            # Code block modified compared to the orignial CIR version:
            # add split arg to keep_bop19, add block for keep_no_gripper
            if only_bop19_test:
                self.frame_index = keep_bop19(self.frame_index, ds_dir, split)

            if only_no_gripper:
                self.frame_index = keep_no_gripper(self.frame_index, ds_dir, split)
            # --------------
    ...
    def __getitem__(self, frame_id):
        ...
        if self.load_depth:
            ...
            camera['orig_depth'] = depth * cam_annotation['depth_scale'] / 1000
            if not self.use_pre_interpolated_depth:
                camera['interpolated_depth'] = lin_interp(camera['orig_depth'])
            # --------------
        else:
        ...
        _, _, camera['orig_depth'], _ = self.resize_augmentation(rgb_orig, mask, camera['orig_depth'], deepcopy(obs))
...
def collate_fn(data):
    *tensors, obs_list = zip(*data)
    ...
    for k,v in obs_list['camera'].items():
        # --------------
        # Code block modified compared to the original CIR version: convert v to numpy array first
