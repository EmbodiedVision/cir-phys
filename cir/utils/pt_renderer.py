...
# https://github.com/princeton-vl/Coupled-Iterative-Refinement/blob/c50df7816714007c7f2f5188995807b3b396ad3d/utils/pt_renderer.py,
...
BOP_DS_DIR = "local_data/bop_datasets"
...
class Pytorch3DRenderer(nn.Module):
    ...
    def load_meshes(self):
        ...
        model_dir = self.ds_dir / self.dataset_name / "models"
        assert model_dir.exists(), f"{model_dir} does not exist"
        ply_filenames = sorted(list(model_dir.rglob("obj_*.ply")))
        ply_filenames = [n for n in ply_filenames if "inertial" not in n.name]
        ...
        meshes = join_meshes_as_batch(mesh_list).to(self.device)
        if (isinstance(meshes.textures, TexturesUV) and
                (torch.stack(meshes.textures.maps_list()) == meshes.textures.maps_padded()).all()):
            meshes.textures._maps_list = None
        # --------------
        return meshes

    def __init__(self, dataset_name: str, device, ds_dir: str = BOP_DS_DIR):
        super().__init__()
        store_attr()
        # --------------
