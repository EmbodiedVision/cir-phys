...
# https://github.com/princeton-vl/Coupled-Iterative-Refinement/blob/c50df7816714007c7f2f5188995807b3b396ad3d/utils/augmentation.py,
...
import PIL
import torch
from cir.crops import crop_to_aspect_ratio, get_K_crop_resize
...
class VOCImageFolder(ImageFolder):
    def __init__(self, root: str):
        super().__init__(root)

    def find_classes(self, directory: str) -> Tuple[List[str], Dict[str, int]]:
        classes = super().find_classes(directory)
        for cl in classes[0].copy():
            if cl in ["Annotations", "ImageSets"]:
                classes[0].remove(cl)
                classes[1].pop(cl)
...
class VOCBackgroundAugmentation(BackgroundAugmentation):
    def __init__(self, voc_root, p=0.3):
        ...
        image_dataset = VOCImageFolder(voc_root)
        # --------------
