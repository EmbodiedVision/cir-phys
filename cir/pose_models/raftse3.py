...
# https://github.com/princeton-vl/Coupled-Iterative-Refinement/blob/c50df7816714007c7f2f5188995807b3b396ad3d/pose_models/raftse3.py,
...
from cir.pose_models.modules.clipping import GradientClip
from cir.pose_models.modules.corr import CorrBlock
from cir.pose_models.modules.extractor import BasicEncoder
from cir.pose_models.modules.gru import ConvGRU
from cir.utils import get_ind, mat2SE3, vectorize_intrinsics
from cir.utils.geom import projective_ops_rgbd as pops
from cir.utils.geom.ba import BA, MoBA
from cir.utils.geom.intersection_ops import generate_pcis, grid_grads
from cir.utils.geom.sampler_utils import bilinear_sampler, sample_depths


# --------------
# Code block added compared to the original CIR version
def visualize_meshes(labels, poses, mesh_lookup):
    import trimesh
    scene = trimesh.Scene()
    for label, pose in zip(labels, poses):
        id = int(label[-6:]) - 1
        mesh = trimesh.Trimesh(mesh_lookup.verts_list()[id].cpu(), mesh_lookup.faces_list()[id].cpu())
        scene.add_geometry(mesh, transform=pose.cpu())
...
class RaftSe3(nn.Module):
    def __init__(self, hdim, cdim, fdim, solver_method, intersection=False, sdfs=None, sample_pcis=False,
                 generator=None, low_res_grid_factor=1):
        super().__init__()
        store_attr()
        ...
        if self.sdfs is not None:
            for k in self.sdfs.keys():
                self.sdfs[k]['sdf_grads'] = grid_grads(self.sdfs[k]['sdf'])

        self.pcis = {k: generate_pcis(torch.tensor(sdf['sdf'].shape, dtype=int) // low_res_grid_factor,
                                      sdf['scale'].unsqueeze(0), sdf['offset'].unsqueeze(0), sampling=sample_pcis,
                                      generator=generator)
                     for k, sdf in sdfs.items()} if intersection and sdfs else None
        # --------------
    ...
    def forward(self, Gs, images, depths_fullres, masks_fullres,
                intrinsics_mat, labels, num_solver_steps, num_inner_loops, renderer, intersection=False, G_bg=None):
        ...
        # --------------
        # Code block added compared to the original CIR version
        if self.intersection and intersection:
            sdfs = [self.sdfs[l] for l in labels]
            pcis = [self.pcis[l] for l in labels]
            if 'bg' in self.sdfs.keys():
                sdfs = [self.sdfs['bg']] + sdfs
                pcis = [self.pcis['bg']] + pcis
            pcis = torch.cat(pcis)
            sdfs = {
                'volumes': torch.stack([s['sdf'] for s in sdfs]),
                'grads': torch.stack([s['sdf_grads'] for s in sdfs]),
                'offsets': torch.stack([s['offset'] for s in sdfs]),
                'scales': torch.stack([s['scale'] for s in sdfs])
            }
        else:
            sdfs = None
            pcis = None
        ...
        for step in range(num_inner_loops):
            ...
            sampled_mask, valid_mask_pixels = bilinear_sampler(masks_fullres[:, jj].unsqueeze(2).float(), target_scaled,
                                                               use_mask=True)
            ...
            if self.solver_method == "Modified BD-PnP":
                ...
                Gs, _, _ = MoBA(target, weight, Gs, depths, intrinsics, num_solver_steps, ii, jj,
                                intersection=self.intersection and intersection, pcis=pcis, sdfs=sdfs,
                                G_bg=G_bg if self.sdfs is not None and 'bg' in self.sdfs.keys() else None)
                # --------------
